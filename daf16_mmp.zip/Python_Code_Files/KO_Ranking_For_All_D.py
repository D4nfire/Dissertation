...
# for testing and not for users. The GOTermIndex value should be changed according
...
def makeGraph(fullGeneDataList):
	...
	while (i < len(fullGeneDataList)):
		G.add_node(fullGeneDataList[i])	# add a node for each gene ID
		i = i + 4
...
def writeResultsToFile(geneAndAllRankingsList, outputFile, testCount, CountPerGene):
	...
	with open(outputFile, "a") as this_file:
		...
		i = 1
		while (i < num2):
			# If all 20 values of d have been tested on (testCount)
			# and the ranking values have not already been written to file (CountPerGene)
...
# calls methods in order required to get the rankings and write results to file.
def runRanking(filepath, GOTermIndex, koGene, graphFile, outputFile, d, geneAndAllRankingsList, testCount, CountPerGene):	
	...
	CountPerGene = writeResultsToFile(geneAndAllRankingsList, outputFile, testCount, CountPerGene);
	
	return geneAndAllRankingsList, testCount, CountPerGene
...
def main(filepath, GOTermIndex, koGene, graphFile, outputFile, geneAndAllRankingsList, testCount, CountPerGene):
	
	i = 0.0
	while (i <= 1.02):
		geneAndAllRankingsList, testCount, CountPerGene = runRanking(filepath, GOTermIndex, koGene, graphFile, outputFile, i, geneAndAllRankingsList, testCount, CountPerGene);
		i = i + 0.05
	

# Call to write the header to the file Rankings.txt	
...
graphPath = "_component_graph.xml"
GOTermIndex = 15
...
while (i < num):
	...
	testCount = 0
	CountPerGene = 0
	main(genePath + testGeneList[i] + endOfPath, GOTermIndex, testGeneList[i], testGeneList[i] + graphPath, "Rankings.txt", geneAndAllRankingsList, testCount, CountPerGene);
	i = i + 1