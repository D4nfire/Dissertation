...
def writeResultsToFile(geneAndAllRankingsList, outputFile, testCount, CountPerGene, fileCount):
	...
	with open(outputFile, "a") as this_file:
		if ((testCount == 21) and (CountPerGene == 0) and (fileCount == 0)):
...
def main(geneAndAllRankingsList, testCount, CountPerGene, fileCount):
	...
	i = 0.0
	while (i <= 1.02):
		geneAndAllRankingsList, testCount, CountPerGene, fileCount = runRanking(filepath, koGene, graphFile, outputFile, i, geneAndAllRankingsList, testCount, CountPerGene, fileCount);
		i = i + 0.05
...
while (i < num):
	...
	testCount = 0
	CountPerGene = 0
	fileCount = main(geneAndAllRankingsList, testCount, CountPerGene, fileCount);
