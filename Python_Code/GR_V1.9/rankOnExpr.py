	...
	with open(filePath) as infile:
		...
		for line in infile:
			...
			# otherwise parse the data to fullGeneDataList
				...
				#fullGeneDataList.append(float(tempList[4]))
			else: 
				...
				while (i < num):
					if (fullGeneDataList[i] == tempList[6]):
						updatedExpr = updatedExpr + fullGeneDataList[i+1]
...
def getNormalisedExprData(fullGeneDataList):
	sumOfEx = 0 
	temp_norm_ex = 0
	num = len(fullGeneDataList)
	...
	i = 0
	while (i < num):
		sumOfEx = sumOfEx + fullGeneDataList[i+1]
		i = i + 2 
	
	j = 0
	while (j < num):
		temp_norm_ex = fullGeneDataList[j+1] / sumOfEx
		fullGeneDataList.insert(j+2, temp_norm_ex)
		num = len(fullGeneDataList)
		j = j + 3
...
def sortByRanking(fullGeneDataList):
	...
	while (i < num):
		...
		geneAndRankList.append(fullGeneDataList[i+2])
...
def main(filePath, koGene, sumOfRanks, outputFile):
	...
	fullGeneDataList = getNormalisedExprData(fullGeneDataList);
