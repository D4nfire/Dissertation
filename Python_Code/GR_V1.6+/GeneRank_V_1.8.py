...
def geneRank(geneIDList, exprDataList, normExprDataList, G, d):
	...
	while ((i < num3) and (converged != True)):
		...
		while (j < num3):
			if (i == 0):
				...
				rankValue = connectionValue + d*(sumOfConnectionList[j])
				rankingValueList[j] = rankValue 
				
			j = j + 1
		
		rankComparisonList.append(rankingValueList)
		
		if (count <= 1): 
			converged = False
			count = count + 1
		elif (bool(set(rankComparisonList[i]) & set(rankComparisonList[i-1]))):
			converged = True
		else:
			converged = False
	...
	print ("The algorithm converged:" , converged , "after" , i-1 , "itterations")
