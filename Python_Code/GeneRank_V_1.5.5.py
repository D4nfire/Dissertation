		...
		while (j < num4):
			if (i != j):
				...
				if (G.degree(gene_ex_r[i]) == 0):
					temp_connection = (hasEdge * (gene_ex_r[i+3])) / 1 
				else:
					temp_connection = (hasEdge * (gene_ex_r[i+3])) / (G.degree(gene_ex_r[i])) 
				...
				rank = (1-d + (d * gene_ex_r[j+4]) 		# this is the ranking of gene j after								
				gene_ex_r[j+4] = rank	# update the ranking		# i iterations
...
rankedList = sortAndPrintRanking(gene_ex_r);
